...
public class MagicService {
	...
	public JSONObject listaPrezzi(JSONArray items) {
		...
		for (int i = 0; i < items.length(); i++) {
			...
			if (error != null) {
				...
				BigDecimal disponibilita = retrieveDisponibilita(Azienda.getAziendaCorrente(), idArticolo);
				if(disponibilita != null)
					prezzoObj.put("disponibilita", disponibilita);
			}
	...
	protected BigDecimal retrieveDisponibilita(String aziendaCorrente, String idArticolo) {
		CachedStatement cs = null;
		ResultSet rs = null;
		try {
			String select = "SELECT (QTA_GIAC_PRM - QTA_ATS_USC_PRM - QTA_PRP_USC_PRM - QTA_ORD_CLI_PRM) AS DISPONIBILITA \r\n"
					+ "FROM THIP.SALDI_BASE sb \r\n"
					+ "WHERE ID_AZIENDA = '"+aziendaCorrente+"'\r\n"
					+ "AND ID_ARTICOLO = '"+idArticolo+"' \r\n"
					+ "AND ID_MAGAZZINO = '001' ";//cablato 001 indicato da verzeletti

			cs = new CachedStatement(select);
			rs = cs.executeQuery();
			if(rs.next()) {
				return rs.getBigDecimal("DISPONIBILITA");
			}
		}catch(Exception e) {
			e.printStackTrace(Trace.excStream);
		}finally {
