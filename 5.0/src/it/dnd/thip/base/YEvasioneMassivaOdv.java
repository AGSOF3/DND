 ...
 * 72452	20/04/2026	AGSOF3	Controllo evasione iniziale documento da piu ordini
 * 72468	04/05/2026	AGSOF3	Calendario aziendale
 */
...
public class YEvasioneMassivaOdv extends BatchRunnable implements Authorizable {
	...
	protected DocumentoVendita cercaDocumentoVenditaAccorpabile(OrdineVenditaRiga ordVenRig,CausaleDocumentoVendita cauDocVen) {
		...
		if(condComp.isGesScontoFineFattura()) {
			...
			if(ordVen.getPrcScontoFineFattura() != null)
				...
				trovaDocVenAccodabile += " IS NULL OR dvt.SCONTO_FINE_FATT = 0) ";
		}
	...
	protected List estraiRigheOrdine() {
		...
		try {
			...
			java.sql.Date dataOrizzonte = cc.aggiungeGiorniLavorativi(TimeUtils.getCurrentDate(), getGiorniOrizzonte());
			ps.setDate(2, dataOrizzonte);
			//72648 >
			rs = ps.executeQuery();
			while(rs.next()) {
